...
export default function App() {
  ...
  const operatorGroupedStats = useMemo(() => {
    ...
    return list.map(name => {
      ...
      const fruitPerformance = ['柚子', '蘋果', '鳳梨', '草莓', '奇異果', '芒果'].map(fName => {
  ...
  // 各水果在不同班別的步留率比較（由真實資料計算）
  const fruitComparisonData = useMemo(() => {
    return FRUITS.map(fName => {
      const row: any = { name: fName };
      operatorGroupedStats.forEach(op => {
        const fp = op.fruitPerf.find(p => p.fName === fName);
        row[op.name] = fp ? fp.yieldRate : 0;
      });
      return row;
    });
  }, [operatorGroupedStats]);
                ...
                <div className="h-96 w-full">
                  <ResponsiveContainer width="100%" height="100%">
                    ...
                    >
                      ...
                      />
                      <Legend verticalAlign="top" iconType="circle" height={36} wrapperStyle={{ fontSize: '11px' }} />
                      <Bar dataKey="食品一班" fill="#F97316" radius={[4, 4, 0, 0]} barSize={16} />
                      <Bar dataKey="食品二班" fill="#27272A" radius={[4, 4, 0, 0]} barSize={16} />
                      <Bar dataKey="食品三班" fill="#71717A" radius={[4, 4, 0, 0]} barSize={16} />
                    </BarChart>
                  </ResponsiveContainer>
                </div>
